const CoolButton = ({ action, setAction }) => {
	...
	return (
		...
		>
			<div
				className='absolute my-[2px] h-[calc(100%_-_4px)] w-1/2 rounded-full bg-main-400 transition-all duration-500 ease-in-out'
			...
			<div className='relative z-20 flex justify-around'>
				<div
					className={`text-center duration-500 ease-in-out ${
						action === "open" ? "text-main-500" : "text-xl font-bold text-white"
					}`}
				>
					List
				</div>
				<div
					className={`text-center duration-500 ease-in-out ${
						action === "open" ? "ml-3 text-xl font-bold text-white" : "text-main-500"
					}`}
				>
					Open
				</div>
