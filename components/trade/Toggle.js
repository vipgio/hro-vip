const Toggle = ({ action, setAction }) => {
	...
	return (
		...
		>
			<div
				className='absolute my-1 h-[calc(100%_-_4px)] w-[calc(50%_+_2px)] rounded-full bg-main-400 transition-all duration-500 ease-in-out'
			...
			<div className='relative z-20 flex justify-around'>
				<div
					className={`text-center duration-300 ease-in-out ${
						action === "receive" ? "text-main-500" : "ml-2 font-bold text-white"
					}`}
				>
					Send
				</div>
				<div
					className={`text-center duration-300 ease-in-out ${
						action === "receive" ? "ml-3 font-bold text-white" : "text-main-500"
					}`}
				>
					Receive
				</div>
