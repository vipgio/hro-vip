...
const CardGallery = React.memo(({ cards, user, filter, selectedCollection, owned, fetchData }) => {
	...
	const searchUser = async (jwt, userId, collectionId, categoryId = 1) => {
	...
	const getLeaderboard = async (collectionId, page) => {
		return http(`${API}/leaderboards/collections/${collectionId}?page=${page}`, {
			method: "GET",
			headers: {
				"Content-Type": "application/json",
				"x-user-jwt": user.jwt,
	...
	const getMarketInfo = async (cardId, page, type) => {
		const { data } = await axios.get(`/api/market/card/${cardId}?page=${page}&type=${type}`, {
			headers: {
				jwt: user.jwt,
			},
		});
		...
		<>
			<div className='ml-1 flex h-full'>
				<div className='flex flex-col sm:block'>
					...
					<span className='inline-flex w-fit items-center'>
						...
						<input
							...
							className='accent-main-500 hover:cursor-pointer'
						/>
					</span>
				</div>
				<div className='ml-auto mr-2 flex flex-col justify-end py-1 sm:block'>
					...
					>
						<FiShoppingCart className='mr-2 text-main-500' />
					...
					<button
						...
						disabled={!selectedCards.length}
					>
						<FiUser className='mr-2 text-main-500' />
